...
| D-05 | Build error surfacing | shipped | Build pipeline failures (Arweave, cnft_dispatch, runImageOps) are hidden from the buyer. UI shows "Still issuing your deed..." indefinitely while sweeper retries. | payments.md §1.3 error codes surface to caller |
...
| D-19 | Arweave Master packaging (ZIP-AES-256 + platform proxy for friendly filename) | **superseded 2026-06-11 by D-21** | (Historical) Arweave-bound payload was a single-layer ZIP-AES-256 archive; FS held raw AES-256-GCM ciphertext and Card 5 decrypted + repackaged into ZIP for Arweave upload. The decrypt-and-rezip round-trip at Card 5 was eliminated by D-21 -- Card 1 now writes the ZIP-AES-256 archive directly to FS, Card 5 lifts it as-is, and `/download-master` returns the password instead of streaming decrypted bytes. ZIP-AES-256 packaging on Arweave is preserved (native-tool extract UX retained). | (resolved by D-21) |
| D-21 | ZIP-at-Card-1 + Card-5 pass-through + password-reveal /download-master | shipped 2026-06-11 | **Card 1 (`POST /v1/images`)** builds the single-layer ZIP-AES-256 archive (`<image_id>.jpg` inside, password = `base64(DEK_image)`) and writes it to `EncryptedMasterStore` at `data/encrypted_masters/<image_id>.zip`. The wrapped DEK is persisted on `images.dek_wrapped`. **Card 5 (`arweave_master.buildAndUpload`)** is now a pure pass-through -- it reads the FS ZIP bytes and uploads them as-is to Arweave (no decrypt, no rezip, no in-memory cleartext). Arweave bytes are byte-identical to FS bytes. **`/download-master`** no longer streams decrypted bytes. It returns JSON `{ archive_url, arweave_uri, password, filename_hint, custody_state }`; the browser shows a password-reveal modal + "Download ZIP from Arweave" button; the buyer fetches the encrypted ZIP from Arweave and extracts client-side with native OS tooling (Windows / macOS Archive Utility / iOS Files / Android Files / Linux unzip 6.0+). The server never holds the cleartext Master in memory after the Card 1 upload buffer is consumed. **D-18 side effect preserved**: first /download-master still peels the outer PLATFORM_DEK wrap of `enc_final`, persists `enc_final_unwrapped`, flips `custody_state` `sealed → unsealed`; the UNSEAL Solana tx co-signed by the buyer's wallet remains the consent record. **INV-02 alignment improved**: server-side decrypt path is eliminated for downloads; the only server-held secrets used are `PLATFORM_DEK` (to unwrap the DEK for password derivation) and `dek_wrapped` (to retrieve the DEK), both of which were already platform-known. **Resale (post-MVP)**: platform will need to extract the ZIP server-side to regenerate the Share Copy for the new owner; contract point at [src/cert/zip.ts `extractFromZipAes256`](../../src/cert/zip.ts) (stubbed). **Migration**: legacy `.bin` files (GCM ciphertext from D-19 era) become orphaned -- the 3 test deeds on the dev machine need re-upload via the new Card 1 flow. Supersedes D-19. | R62 §2.3 "AES-256-GCM" text for the Arweave payload (mode shift only; algorithm + key unchanged from D-19's framing) |
| D-22 | Master Download Notice (DLN) click-wrap gates the seal-break | shipped 2026-06-11 | First `/v1/deeds/:imageId/download-master` requires body `{ initials: string }`. The server captures a DLN (Master Download Notice) signature via `cert/esign.captureSignature` BEFORE the seal-break runs -- Signature row + Solana-tx Memo co-signed by the buyer's wallet per esign.md §2.8. The buyer-facing UI shows a warning modal with the consequences (irreversible `sealed → unsealed`, no Platform-facilitated resale), an initials input, and an acknowledgement checkbox. Submit fires the POST; on success the password-reveal modal opens (D-21). **Why**: prevents accidental clicks on the Download button from triggering the one-way custody transition. **Doc**: `legal/download_notice.md` + binder entry `dln7t` with `props_schema = { image_id, initials, owner_wallet_address }`. **DocumentType enum extended**: `cert/esign.ts` + `cert/legal.ts` add `DLN`. **Idempotency**: subsequent calls (already-unsealed deeds) skip DLN capture -- the consent record is one-shot per owner-tenure. **Error codes**: `INITIALS_REQUIRED` (empty initials on first call), `INITIALS_TOO_LONG` (> 10 chars), `DLN_SIGNATURE_FAILED` (signature capture failed), `DLN_DOC_NOT_FOUND` (legal binder misconfigured). | R62 §3.5.1 download trigger silently transitions custody; D-22 inserts an explicit click-wrap step + on-chain consent record |
| D-20 | Share-copy download endpoint never built; image-page download serves Master | shipped | R71 §3.7 row 20 specifies `GET /v1/deeds/:mintAddress/social-copy-download` -- a separate endpoint for downloading the watermarked Share Copy with ownership verification. **Not built.** The single download CTA on the image page calls `POST /v1/deeds/:imageId/download-master` (the existing Master download per R62 §3.5.1 seal-break + D-18). Share Copy is generated at Card 5 and stored on Cloudinary for display purposes only (R62 §7.6 watermark variants); not served via a separate authenticated endpoint. "Social copy" terminology dropped throughout in favor of "Share Copy" (R62 §7.6 native name). `docs/commerce/renderer.md` describing the phantom endpoint was deleted. | R71 §3.7 row 20 (endpoint dropped); R62 §3.5 "social copy" terminology updated to "Share Copy" |
...
**Why.** The buyer's intent at this click is "I want this image, with these letters". Deed mint, Arweave upload, and cnft_dispatch are platform-side side effects whose success or failure should not change the buyer's mental model of what just happened. "Mark my image" frames the click as a customer action; "Issue deed" frames it as a blockchain transaction the buyer is initiating. The former is closer to how the platform wants the buyer to think of the transaction (per R67 §5.8 deed-as-receipt).
...
**What we ship.** Build-pipeline errors (Arweave upload, cnft_dispatch, runImageOps failures) are caught and logged server-side but **not surfaced to the buyer**. The buyer sees one of three states on the post-mark listing page:
...
**Migration status.** Existing Crossmint-issued test data was wiped 2026-06-03 (zero real users). Production migration policy (OI-06 in deed.md) is therefore moot for this cohort. Retired implementation files (crossmint_dispatch.ts, crossmint_webhook.ts, crossmint_lookup.ts and their docs) are deleted -- only the migration record (this D-14 entry + adr_0008) survives in repo. The .env vars CROSSMINT_API_KEY + VITE_CROSSMINT_CLIENT_KEY are commented out but not deleted -- next .env audit.
...
| R62 + R71 pending alignment | /docs/r62_r71_alignment.md |
