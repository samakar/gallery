...
| Public image page with Listing-preview rendering and OG / Twitter Card; render branches on (`images.visibility`, sold-status) per §2.7: pre-sale shows the live listing; post-sale-private shows the unwatermarked Thumbnail + Certificate of Authenticity (CoA) panel + sold-date strip (the certificate is the public face of the work even when image content is private; technical on-chain deed metadata stays behind "View deed"); post-sale-public (after owner clicks Share per §2.6) shows the full Share Copy surface | Yes |
...
| Owner Share Copy download from Collection at the 1080px social-native format (JPEG; in-pixel URL text and monogram embedded -- monogram default from the buyer's Stripe billing initials, buyer-overridable). Keepsake Copy, buyer-selectable print sizing, and print fulfillment all deferred to MMP | Yes |
| Owner Master Image download (platform-mediated decryption + delivery per R62 §3.5.1). The deed-holder clicks Download; the server fetches the encrypted Master from Arweave, unwraps `DEK_image` from `platform_DEK`, decrypts the Master bytes, and streams them with `Content-Disposition: attachment` (force save dialog). The deed_state transitions from `sealed` to `unsealed` (locally; on-chain `update_metadata_v1` sync of the state field is post-MVP per /docs/divergences.md). Subsequent downloads are idempotent: re-streams the Master without further state mutation. Resale capability disabled on `unsealed` is forward-binding (resale itself is post-MVP) | Yes |
...
| Transactional email for the R62 §3.5 legal-artifact subset: (a) onboarding email to creator at sign-cma carrying the executed CMA PDF; (b) onboarding email to buyer at MJA capture (first purchase) carrying the executed BMA / MJA PDF; (c) COA email at first-sale deed mint to both creator and buyer carrying the four-PDF certification bundle (Certificate of Authenticity, Title Document, Purchase Receipt, Per-image License Acceptance record). Postmark as the ESP per ADR-0009. Spec at /docs/cert/email.md | Yes |
...
### 1.2 Out of MVP Scope

The following surfaces are explicitly NOT in the MVP and MUST NOT be considered required for launch. They are listed here so the scope is unambiguous and so that future work does not get classified as "MVP polish" when it is not.

| Surface | Why deferred | Notes / forward-binding |
|---|---|---|
| Resale flow (Card 8 / R71 §2.4 resale continuation) | Out of MVP. INV-05's 30-day post-purchase settlement window gives natural grace before any first resale would be valid; the resale UI + the cnft_dispatch transfer + `enc_final` re-key path can ship after MVP launch. | When implemented: extend cnft_dispatch with `transferV2` + `update_metadata_v1` for enc_final re-key under the Bubblegum V2 PermanentTransferDelegate (HOT_RESALE_KEY signs). |
| Email fan-out beyond the R62 §3.5 legal-artifact subset: COA at resale, takedown-decision notifications, abuse-report acknowledgements, Share Copy-ready notifications, generic transactional pings | Out of MVP. The corresponding workflows (resale, takedown, the moderation queue UI) are themselves deferred per this §1.2, so the email surfaces for them are scoped out with their parents. The R62-mandatory onboarding + first-sale COA emails ARE in MVP per §1.1. | When the parent workflows ship, the email module from §1.1 already exists -- adding a new template variant + hook point is the only work. Templates spec'd in /docs/cert/email.md §3.2. |
| Procedural multisig admin tool for INV-06 deed_state mutations (disputed / void; `void → burned` is sweeper-driven, no multi-sig) | Out of MVP -- because the events the tool gates (takedowns, dispute resolutions, fraud-driven terminations) are themselves out of MVP scope. At MVP single HOT_OPS_KEY signs all metadata updates server-side, with no INV-06 transitions occurring. | See /docs/registry/deed.md OI-01. Needs to ship before the first real INV-06 transition occurs. Until then, INV-06's enforcement is forward-binding (it engages when the transition is attempted, not at MVP launch). |
| Takedown handler (DMCA, Take It Down Act, court-ordered removals, NCMEC referrals beyond Card 1 ingestion gate) | Out of MVP. The Card 1 ingestion moderation gate (Tier 0 + Tier 1) prevents the worst content from reaching the platform; post-mint takedown is a compliance feature for the production deployment. | When implemented: /docs/cert/takedown.md is the spec; flips deed_state via the OI-01 procedural multisig tool. |
| Creator-side Stripe Connect Express onboarding + automated payouts | Out of MVP. Platform holds creator's 90% net in Elanoid's Stripe balance; manual payout. | Already noted in §1.1 in-scope table. |
| Master Image on-chain deed_state sync (DB → chain via `update_metadata_v1`) | Out of MVP. At MVP the `sealed`→`unsealed` transition is DB-only when the buyer downloads the Master; the on-chain `deed_state` field stays at `sealed` until the on-chain sync ships. The DB is the source of truth at MVP. | Divergence recorded in /docs/divergences.md. Requires DAS proof fetching + HOT_OPS_KEY signing of `update_metadata_v1`. |
| Keepsake Copy, print fulfillment, buyer-selectable print sizing | Out of MVP -- Share Copy + Master download are the deed-holder artifacts at MVP. | Already noted in §1.1 in-scope table. |
| Saved card on returning purchase | Out of MVP. Card re-entered on every purchase via Stripe Embedded Checkout. | Already noted in §1.1 in-scope table. |
| C2PA manifest authoring, spectrographic / invisible watermark | Out of MVP -- deferred to MMP per /docs/deferred/drm_c2pa.md and /docs/deferred/drm_spectrographic.md. | Embedded provenance manifests in Master + Share Copy (REQ-MINT-04 in /docs/registry/deed.md) are also post-MVP for the same reason. |
| Public Creator Page (aggregated landing for a creator's listings) | Out of MVP -- deferred per §3.4. Image pages link to creator name as plain text without a routable creator landing. | |
| Account / Settings page | Out of MVP. | |
| Per-event Arweave tree-state snapshots for 200-year permanence (REQ-MINT-04) | Out of MVP. The deed exists and is provable on-chain without snapshots; snapshots are belt-and-suspenders for tree-wide reconstruction under simultaneous DAS-provider failure. | See /docs/registry/deed.md OI-12. |
| Hardware-wallet custody of COLD_RECOVERY_KEY | Out of MVP -- operational hardening for production deployment. At MVP all keys including COLD_RECOVERY_KEY are server-held keypairs, sufficient for dev / staging. | See /docs/registry/deed.md OI-04. |
| Distributed mint mutex (Redis / Postgres advisory lock) | Out of MVP. In-process Node mutex shipped in cnft_dispatch; sufficient for single-instance API. | See /docs/registry/deed.md OI-10. |
| Tree sharding policy for throughput beyond single-mutex capacity | Out of MVP -- single tree handles the entire MVP volume. | See /docs/registry/deed.md OI-13. |
...
**MVP delivery (ADR-0007 + /docs/divergences.md D-01, D-02).** The buyer-active portion (steps 1-9 below) is rendered as a single modal -- the **BuyWizard** -- mounted at Buy-button click and persisting through the Magic OAuth round-trip via sessionStorage state + a `?buy=resume` return URL. The progress strip reads `1. Sign in -> 2. Sign documents -> 3. Payment -> 4. Personalize your edition`. The monogram-confirm button reads **"Mark my image"**, not "Issue deed" -- framing the click as a customer marker rather than a blockchain action the buyer is launching. Once the buyer clicks Mark my image, the wizard closes and the platform-active portion (steps 10-17) proceeds in the background per ADR-0007 (buyer-friendly retry model: server retries indefinitely on infrastructure failure; buyer-facing UI shows "Issuing your deed..." through the wait; the buyer never sees a Retry button).
...
| 9 | Monogram personalization -- BuyWizard step 4 / **"Mark my image"** click (D-02) | Web App + Backend | The buyer is shown the Share Copy monogram with the default initials derived live from the current Checkout Session's `billing_details.name` (no platform-stored seed), plus the creator name, rendered per the R62 §2.2 monogram typography (restrained gallery-register typeface and color treatment; human-readability and aesthetics optimized, outside the §7.6 URL-text OCR contract). The buyer may keep the default or override it with custom text; the chosen monogram is captured in `purchases.monogram_text` for the Share Copy build at step 13. Full artist credit is retained regardless of the monogram choice. The BuyWizard closes the moment the buyer clicks Mark my image; the listing page renders the owner state with action buttons disabled while steps 10-14 run. Per ADR-0007, if the buyer closes the tab between step 7 (payment authorized) and step 9 (Mark click), the row sits at `status='paid', monogram_text=null` indefinitely. On the buyer's next signed-in visit to the image page, `GET /v1/images/:imageId` surfaces `pending_purchase_id` and the BuyWizard auto-opens at the monogram step |
...
| 16 | In-platform purchase confirmation | Web App | The buyer has been on the owner-state listing page since step 9 (BuyWizard closed at the Mark click). Receipt, License Acceptance, deed details, and Collection link surface in the Deed of Ownership panel (CoA panel for authenticity) on that same page; the "Issuing your deed..." status flips to the Solscan link as soon as steps 11-14 complete (ADR-0007 retries on infrastructure failure transparent to buyer per D-05). Stripe sends its own payment receipt automatically; no platform-sent email |
...
The same BuyWizard mounts. The welcome step is skipped (active Magic session detected via localStorage); the wizard opens directly at step 2 (contracts), and the contracts step renders only the License Acceptance tab (no MJA tab; the bundled status check at `GET /v1/me/esign-status` returns `has_mja: true`).

| Step | Action | System | Notes |
|---|---|---|---|
| 1 | Click "Buy" on image page | Web App | If Magic SDK session is active, BuyWizard opens at step 2 (contracts); if signed out, BuyWizard opens at step 1 (welcome) and the OAuth round-trip resumes the wizard via `?buy=resume` on return |
| 2 | OAuth re-authenticate if signed out | Magic SDK | Same embedded wallet recovered deterministically from OAuth identity; no new wallet provisioned. Returns through `/auth/callback` to `/<image_id>?buy=resume`, BuyWizard re-mounts at step 2 (contracts) |
| 3 | Per-image License Acceptance click-wrap | Web App + Backend | Single document; new signing_event_id, new image_hash; MJA skipped (already on file in signatures table); rendered in the BuyWizard's contracts step with only the License tab visible |
...
**Private surface rendering.** Anonymous visitors to `epimage.com/<image-id>` when (`images.visibility = 'private'` and a `purchases` row exists for the image) see a Gallery page identical in chrome to the live image page (header, footer, branding) with a metadata-only composition: the unwatermarked **Thumbnail** (R62 §2.2 Thumbnail variant, ≤500 px long-edge, no watermark, no monogram, no creator credit overlay) presented inside a **two-tone gallery frame** (thick dark outer frame stripe + lighter inner mat stripe, with a drop shadow evoking a print hung on a wall) as the hero artifact; the **Certificate of Authenticity (CoA) panel** rendering the certificate fields per R62 §3.4 (creator name, title, edition tier, creation date, edition number, creator ESIGN affirmation signature record); and a **sold-date strip** (e.g., "Sold March 5, 2026"). The Share Copy itself is not rendered -- the image content stays in the owner's private custody until the owner toggles visibility to public. The technical on-chain deed metadata (mint address, sha256, phash, Arweave URI, deed_state) is NOT rendered on this surface; it stays behind the separate "View deed" route. The metaphor: a framed print on a museum wall behind glass, catalog label visible alongside. No Buy button (the deed is sold), no owner attribution (owner identity stays private), no Report button on this surface. The page is served with `<meta name="robots" content="noindex,nofollow">` and the OG / Twitter Card metadata renders generic Gallery branding rather than image-specific previews.
...
| `status` | TEXT | `pending_review` \| `draft` \| `live` \| `sold` \| `taken_down`; forward-compat: `disputed`. New uploads land at `pending_review` until the founder approves (§2.2 step 5); transitions to `draft` on approval, then to `live` on §2.2 step 9 publish |
...
| `custody_state` (DB column `deed_state` via `@map`) | TEXT | Custody axis: `sealed` initial at mint; `unsealed` after the deed-holder's first /download-master (D-18 seal-break); forward-compat `burned` (voluntary owner-burn OR sweeper-driven post-void). Mapped via Prisma `@map("deed_state")` for migration backward-compat. |
| `legal_state` | TEXT | Legal axis: `legit` default; forward-compat `disputed` (multi-sig adjudicating) and `void` (adjudicated against; in compliance hold) -- both MMP only. |
...
| `disputed` (forward-compat) | DMCA, Take It Down Act, or RoP notice | Reserved; not active at MVP, since founder uses `taken_down` as a catch-all manual route |
...
Forward-compat: at MMP the `confirmed` state extends with `sealed` and `unsealed` substates aligned to R62 §3.5.1 image-custody semantics (the `deed_state` field records whether the image is in platform-mediated custody or has been extracted by the holder). At MVP every `confirmed` purchase corresponds to a `sealed` image (the image is in platform custody; deed-holder Master download is deferred to MMP).
...
| Deed-state correctness | Zero deeds in an incorrect state (sealed / unsealed / disputed / void / burned mismatch) | Manual audit at measurement-window end against Solana on-chain state |
...
| Deed-State Mutations | Bubblegum V2 `update_metadata_v1` (signed by HOT_OPS_KEY as tree authority) for `disputed` / `void` transitions; on-chain leaf-removal via Bubblegum `burn` instruction is per-case operational policy at termination (recorded in `termination_method` audit field per deed.md OI-15), not encoded as a separate state | sealed / unsealed / disputed / void / burned transitions per /docs/registry/deed.md §2.3; multi-sig deferred to post-MVP per OI-03 |
...
| | Two orthogonal deed state machines (custody: sealed/unsealed/burned; legal: legit/disputed/void; single-signer at MVP) | 6 |
...
model Image {
  ...
  status              String                                          // pending_review | draft | live | sold | taken_down | disputed
...
model Deed {
  ...
  ownerId            String   @map("owner_id")
  custodyState       String   @default("sealed") @map("deed_state")   // sealed | unsealed | burned
  legalState         String   @default("legit")                        // legit | disputed | void
