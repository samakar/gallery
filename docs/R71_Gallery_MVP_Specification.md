...
| Owner Master Image download (platform-mediated decryption + delivery per R62 §3.5.1). The deed-holder clicks Download; the server fetches the encrypted Master from Arweave, unwraps `DEK_image` from `platform_DEK`, decrypts the Master bytes, and streams them with `Content-Disposition: attachment` (force save dialog). The deed_state transitions from `sealed` to `opened` (locally; on-chain `update_metadata_v1` sync of the state field is post-MVP per /docs/divergences.md). Subsequent downloads are idempotent: re-streams the Master without further state mutation. Resale capability disabled on `opened` is forward-binding (resale itself is post-MVP) | Yes |
...
| Procedural multisig admin tool for INV-06 deed_state mutations (rights-disputed / void / burned) | Out of MVP -- because the events the tool gates (takedowns, dispute resolutions, fraud-driven burns) are themselves out of MVP scope. At MVP single HOT_OPS_KEY signs all metadata updates server-side, with no INV-06 transitions occurring. | See /docs/registry/mint_architecture.md OI-01. Needs to ship before the first real INV-06 transition occurs. Until then, INV-06's enforcement is forward-binding (it engages when the transition is attempted, not at MVP launch). |
...
| Master Image on-chain deed_state sync (DB → chain via `update_metadata_v1`) | Out of MVP. At MVP the `sealed`→`opened` transition is DB-only when the buyer downloads the Master; the on-chain `deed_state` field stays at `sealed` until the on-chain sync ships. The DB is the source of truth at MVP. | Divergence recorded in /docs/divergences.md. Requires DAS proof fetching + HOT_OPS_KEY signing of `update_metadata_v1`. |
...
| C2PA manifest authoring, spectrographic / invisible watermark | Out of MVP -- deferred to MMP per /docs/deferred/drm_c2pa.md and /docs/deferred/drm_spectrographic.md. | Embedded provenance manifests in Master + Share Copy (REQ-MINT-04 in /docs/registry/mint_architecture.md) are also post-MVP for the same reason. |
...
| Per-event Arweave tree-state snapshots for 200-year permanence (REQ-MINT-04) | Out of MVP. The deed exists and is provable on-chain without snapshots; snapshots are belt-and-suspenders for tree-wide reconstruction under simultaneous DAS-provider failure. | See /docs/registry/mint_architecture.md OI-12. |
| Hardware-wallet custody of COLD_RECOVERY_KEY | Out of MVP -- operational hardening for production deployment. At MVP all keys including COLD_RECOVERY_KEY are server-held keypairs, sufficient for dev / staging. | See /docs/registry/mint_architecture.md OI-04. |
| Distributed mint mutex (Redis / Postgres advisory lock) | Out of MVP. In-process Node mutex shipped in cnft_dispatch; sufficient for single-instance API. | See /docs/registry/mint_architecture.md OI-10. |
| Tree sharding policy for throughput beyond single-mutex capacity | Out of MVP -- single tree handles the entire MVP volume. | See /docs/registry/mint_architecture.md OI-13. |
