...
| Trust maintenance | public Report intake, dispute handling, two-phase termination: `disputed` (multi-sig adjudicating) → `void` (adjudicated invalid; preservation hold; platform-mediated access blocked) → `burned` (sweeper executed deletion after per-reason grace expires; Cloudinary + local-disk + DB keys destroyed; optional Bubblegum leaf-burn for CSAM-class cases) | exit side mirrors the CSAM -> NCMEC entry side |
...
- Deed has **two orthogonal state machines** (per deed.md §2.3): **custody axis** (`sealed` → `unsealed` → `burned`) tracks what the platform can do with the bytes; **legal axis** (`legit` → `disputed` → `void`) tracks adjudication outcome. MVP mints every deed `custody=sealed + legal=legit`. First `/download-master` transitions `custody: sealed → unsealed` (Master extracted; D-18 side effect persists `enc_final_unwrapped`). MMP-only legal pipeline: `legit → disputed → void` under 3-of-5 multi-sig; then the termination sweeper transitions `custody → burned` after the per-reason compliance hold expires (CSAM 90d, DMCA 14d, etc.). Owner voluntary burn skips the legal pipeline entirely: `custody → burned` directly with `legal` staying `legit`, owner wallet signature substituting for multi-sig.
...
| R62 §3.5.1 | deed state machines (two axes per deed.md §2.3: custody = sealed/unsealed/burned; legal = legit/disputed/void; R62's single-column model is split for orthogonality) |
