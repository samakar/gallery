...
`to`: single email address (creator). `cma`: the props for the CMA binder entry (matches `binder.entries[cma].props_schema`, empty `{}` at MVP). Idempotency key defaults to `onboarding_creator:<signature_id>`.
...
`to`: array of TWO addresses (creator + buyer; Postmark batches into one envelope). The four PDF prop bundles (`coa`, `title_document`, `purchase_receipt`, `sal`) carry the per-PDF props -- COA and SAL types per their `binder.entries[type].props_schema` (see [legal_binder.md §2.3](legal_binder.md)); title_document + purchase_receipt per [registry/deed.md](../registry/deed.md). Idempotency key defaults to `coa:<image_id>`.
...
PDF rendering specs: [legal_binder.md §2.3](legal_binder.md) for binder docs; [registry/deed.md](../registry/deed.md) for operational docs. At a glance, the four COA attachments are:
...
| `src/cert/pdf_bundle.tsx` | The PDF render functions for binder docs (CMA, MJA, COA, SAL) + operational docs (Title Document, Purchase Receipt) + shared subcomponents (`DocumentHeader`, `DataRow`, `Footer`). Single-file mirror of email_templates.ts -- consumed by the typed senders in email.ts, not called directly anywhere else. |
...
Symmetry: email content lives in `email_templates.ts`, PDF content lives in `pdf_bundle.tsx`. Both are template-only files (no I/O, no DB); the integration logic stays in `email.ts`. PDF rendering is mechanical conversion -- per-doc content + props are defined in [legal_binder.md §2.3](legal_binder.md) (binder docs) and [registry/deed.md](../registry/deed.md) (operational docs).
...
| legal_binder.md §2.3 + registry/deed.md | PDF render specs; produces the attachments this module sends |
