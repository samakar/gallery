# DRM Content Uniqueness Gate

Two-tier gate (pHash + DINOv2). Prevents creator self-duplication and surfaces cross-creator duplication for §7.1.7 Provenance and Rights Verification (not in MVP scope; flagged as downstream gate).
...
### 2.1 Tier 1: Perceptual Hash
Compute 64-bit pHash (Zauner 2010, DCT-of-luminance). Visually similar images produce similar hashes regardless of compression, scaling, or minor color shifts. Hamming distance < 10 indicates high similarity.

### 2.2 Tier 2: Neural Embedding
Compute DINOv2 dense feature vector (Oquab et al. 2024, Meta AI; ViT trained on 142M curated images). Captures compositional / stylistic similarity that pHash misses.
...
| Dependency | Role |
|---|---|
| pHash WASM library | hash computation (shared with client-side batch pre-check per image_spec §3.1) |
| DINOv2 (Meta AI) model | embedding generation |
| Vector store (e.g., pgvector, Qdrant) | per-creator and platform-wide ANN queries |
...
| drm_authenticity | downstream reverse-image consumer |
| image_spec §3.1 | client-side pHash batch pre-check (advisory) |
| card1_certify_wsd | calling workflow |
| R62 §7.1.2 | reference architecture |
